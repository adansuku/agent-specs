...
- **`context/` como contenedor único de memoria** (ADR-0003, NOVA-9): toda la memoria
  arquitectónica (ADRs, CONTEXT.md, specs, cambios en curso) vive bajo
  `context/`. Renombrado desde `.docs/` para mayor visibilidad. La alternativa anterior (`openspec/`) se eliminó por ser
  redundante.
